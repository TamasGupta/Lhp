...
import SearcIcon from '../assets/icons8-search.gif'
interface File {
  id: string;
  name: string;
  path: string;
...
export const FileList: React.FC<FileListProps> = ({
  ...
  const viewPDF = async (path: string, fileName: string) => {
    try {
      const {
        data: { publicUrl },
      } = await supabase.storage.from("files").getPublicUrl(path);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-3 ">
      <div className="w-full flex rounded-full mb-5 items-center overflow-hidden">
        <div className="w-10 h-10 rounded-full  flex items-center justify-center">
          <img src={SearcIcon} alt="search" className="w-8 h-8 rounded-full mix-blend-multiply" />
        </div>
        <input
          type="text"
          onChange={(e) => setSearch(e.target.value)}
          className="w-[90%] p-3 rounded-full bg-[#4E54C8] focus:outline-none bg-opacity-20 placeholder:text-[#4E54C8]"
      ...
      <div className="bg-white shadow-lg rounded-lg overflow-hidden">
        <div
          className={`divide-y divide-gray-200 ${isdarkMode && "bg-slate-600"}`}
        >
          {files.map((file) => (
            <div
              key={file.id}
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {file.type === "application/pdf" && (
                  ...
                  >
                    <Eye
                      className={`w-4 h-4 ${isdarkMode && "text-slate-300"} `}
                    />
                    <span
                      className={`${
